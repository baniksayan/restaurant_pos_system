...
class BillingProvider extends ChangeNotifier {
  ...
  Future generateBill({
    ...
    try {
      ...
      debugPrint('BillingProvider - Subtotal: ${CurrencyConstants.symbol}$subtotal');
      debugPrint('BillingProvider - GST Amount: ${CurrencyConstants.symbol}$gstAmount');
      ...
      if (orderId != null && orderId.isNotEmpty) {
        ...
        debugPrint('BillingProvider - Calling GetOrderDetalForBill API with orderId: $orderId');
        
        final orderDetailResponse = await ApiService.getOrderDetailForBill(orderId: orderId);
        ...
        if (orderDetailResponse?.isSuccess == true && orderDetailResponse?.data != null) {
          ...
          debugPrint('Total Price: ${CurrencyConstants.symbol}${orderDetail.totalPrice}');
          debugPrint('Order Detail ID: ${orderDetail.orderDetailId}');
        } else {
          debugPrint('BillingProvider - ERROR: Failed to get order details for bill');
      ...
      String customerId = "00000000-0000-0000-0000-000000000000"; // Default customer ID
      if (orderDetail != null && orderDetail.customerPhoneNo.isNotEmpty) {
        debugPrint('=== STEP 2: GET CUSTOMER BY MOBILE ===');
        debugPrint('BillingProvider - Calling getCustomerByMobileNo API with: ${orderDetail.customerPhoneNo}');
        
        final customerResponse = await ApiService.getCustomerByMobileNo(contactNo: orderDetail.customerPhoneNo);
        ...
        if (customerResponse?.isSuccess == true && customerResponse?.data != null) {
          ...
          debugPrint('Customer First Name: ${customerResponse.data!.customerFirstName}');
          debugPrint('Customer Last Name: ${customerResponse.data!.customerLastName}');
        } else {
          debugPrint('BillingProvider - Customer not found or error, using default customer ID');
        }
      } else {
        debugPrint('BillingProvider - No customer phone provided, using default customer ID');
      }
      ...
      if (orderId != null && orderId.isNotEmpty && orderDetail != null) {
        ...
        debugPrint('BillingProvider - Creating bill with customerId: $customerId');
        ...
        debugPrint('BillingProvider - CreateBill Request Body (with orderDetail):');
        debugPrint(createBillRequest.toJson().toString());

        final billResponse = await ApiService.createBill(request: createBillRequest);
        ...
        if (billResponse?.isSuccess == true) {
          ...
          debugPrint('Generated Bill No: ${billResponse.data?.generatedBillNo}');
          debugPrint('Bill No: ${billResponse.data?.billNo}');
          debugPrint('Return To Customer: ${CurrencyConstants.symbol}${billResponse.data?.returnToCustomer}');
        } else {
        ...
        debugPrint('BillingProvider - Using fallback bill creation (no orderDetail)');
        debugPrint('BillingProvider - OrderId: $orderId, OrderDetail: ${orderDetail != null}');
        ...
        final createBillRequest = CreateBillRequest(
          ...
          discountPercBillHd: 0,
          customerIdUI: customerId,
          itemList: "", // Empty for now - might need to generate from cart items
          splDisPer: 0,
          splDisReason: "",
          outletId: HiveService.getOutletId() ?? 1,
          billPrefix: "BL",
        ...
        if (billResponse?.isSuccess == true) {
          ...
          debugPrint('BillingProvider - ERROR: Failed to create fallback bill via API');
          throw Exception('Failed to create bill via API');
        }
      }
